...
run:
	docker build -t highloadcup .
	docker run -m=2G --memory-swap=2G -p 8000:80 -p 27017:27017 -v $(CURDIR)/test_accounts_220119/data:/tmp/data --rm -t -i highloadcup

test:
	docker build -t highloadcup .
	docker run -m=2G --memory-swap=2G -v $(CURDIR)/test_accounts_220119/data:/tmp/data --rm -t highloadcup "test app -v"
...
tester_run: 
	docker build -t highloadcup .
	docker run -m=2G --memory-swap=2G -p 8000:80 -p 27017:27017 -v $(CURDIR)/test_accounts_220119/data:/tmp/data --rm --name highloadcup-tester-run -d highloadcup
	sleep 10
	...
	# highloadcup_tester -addr http://127.0.0.1:8000 -hlcupdocs ./test_accounts_220119/ -test -tank 400
